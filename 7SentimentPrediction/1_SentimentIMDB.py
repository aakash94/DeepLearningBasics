...
## words is the list here
count_word = Counter(words)
vocab = sorted(count_word, key=count_word.get, reverse=True)
vocab_to_int = {word: i for i, word in enumerate(vocab, 1)}

## use the dict to tokenize each review in reviews_split
## store the tokenized reviews in reviews_ints
reviews_ints = []
for single_review in reviews_split:
    reviews_ints.append([vocab_to_int[word] for word in single_review.split()])
...
test_id = int(len(remaining_x) * 0.5)
val_x, test_x = remaining_x[:test_id], remaining_x[test_id:]
val_y, test_y = remaining_y[:test_id], remaining_y[test_id:]
